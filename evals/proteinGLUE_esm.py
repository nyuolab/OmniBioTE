...
def format_sequence_and_label(sequence, label, sp):
    ...
    """
    tokenized_sequence = sp(sequence).input_ids
...
def get_training_sets(task, dataset, sp, format_func):
    """
    ...
    if task not in ["CB513SS3", "CB513SS8"]:
        X_train, y_train = process_data(
            sp, X_train_raw, y_train_raw, format_func
        )
        X_val, y_val = process_data(
            sp, X_val_raw, y_val_raw, format_func
        )
    else:
    ...
    X_test, y_test = process_data(
        sp, X_test_raw, y_test_raw, format_func
    )
...
def finetune_on_task(
    ...
    head = torch.nn.Linear(
        base_model.embeddings.word_embeddings.weight.shape[-1], int(output_dim)
    ).to(device).to(dtype)

    # Parameter groups for different LR
    param_groups = [
        {"params": [p for n, p in base_model.named_parameters() if "embeddings" in n], "lr": embed_lr},
        {"params": [p for n, p in base_model.named_parameters() if "embeddings" not in n], "lr": lr},
...
def test_model(task, model, head, dataset, sp, format_func, metric, loss_str, device, dtype=torch.bfloat16):
    ...
    with torch.no_grad():
        for i in range(len(X_test)):
            ...
            embeddings = model(x_tensor).last_hidden_state
...
def run_experiment(
    ...
    print(f"Batch size {batch_size}, LR = {finetuning_lr}")
    print(f"Output suffix = {output_suffix}")
...
def parse_args():
    ...
    parser.add_argument("--model_size", type=str, required=True, help="Path to the PyTorch model file.")
    parser.add_argument(
        "--extra_pretrain",
        action="store_true",
