...
class Tokenizer:
    ...
    def DecodeAsIds(self, token_ids):
        vocab = self.vocab
...
def finetune_on_task(
    ...
    head = torch.nn.Linear(
        base_model.transformer.wte.weight.shape[-1], int(output_dim)
    ).to(device).to(dtype)
    ...
    for step in pbar:
        ...
        for _ in range(num_accumulation_steps):
            ...
            X_batch = [x[:2048] for x in X_batch]
            Y_batch = [y[:2047] for y in Y_batch]
...
def test_model(task, model, head, dataset, sp, format_func, metric, loss_str, device, dtype=torch.bfloat16):
    ...
    with torch.no_grad():
        for i in range(len(X_test)):
            x_test = X_test[i][:2048]
            y_test = Y_test[i][:2047]
    ...
    if extra_pretrain:
        ...
        for task in tasks:
            ...
            X_train = get_training_sets(
                task, dataset, sp, format_sequence_and_label, prefix,
            )[0]
