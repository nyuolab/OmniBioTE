...
class Tokenizer:
    ...
    def Encode(self, sequence):
        ...
        return [vocab.get(tok, UNKNOWN_TOKEN) for tok in tokens_raw]
...
def run(args):
    ...
    print(f"Running on rank {rank}, device {device}, node name {node_name}")
    ...
    tokenizers = {"DNA": Tokenizer("DNA"), "peptide": Tokenizer("peptide")}
    ...
    if args.train_type == "protein":
        train_dataset = OmniDataset([{"path": os.path.join(base_dir, "uniref100/train"), "type": "peptide", "fraction": 1.0, "offset": 0}],
                                    ctx_len=args.ctx_len, tokenizers=tokenizers)
        test_dataset = OmniDataset([{"path": os.path.join(base_dir, "uniref100/val"), "type": "peptide", "fraction": 1.0, "offset": 0}],
                                    ctx_len=args.ctx_len, tokenizers=tokenizers)
    elif args.train_type == "nucleotide":
        train_dataset = OmniDataset([{"path": os.path.join(base_dir, "genbank/train"), "type": "DNA", "fraction": 1.0, "offset": 0}],
                                    ctx_len=args.ctx_len, tokenizers=tokenizers)
        test_dataset = OmniDataset([{"path": os.path.join(base_dir, "genbank/val"), "type": "DNA", "fraction": 1.0, "offset": 0}],
                                    ctx_len=args.ctx_len, tokenizers=tokenizers)
        
    elif args.train_type == "mixed" or args.train_type == "halfnhalf":
        train_dataset = OmniDataset([{"path": os.path.join(base_dir, "genbank/train"), "type": "DNA", "fraction": 0.8 if args.train_type == "mixed" else 0.5, "offset": 0},
                                     {"path": os.path.join(base_dir, "uniref100/val"), "type": "peptide", "fraction": 0.2 if args.train_type == "mixed" else 0.5, "offset": 0}],
                                    ctx_len=args.ctx_len, tokenizers=tokenizers)
        test_dataset = OmniDataset([{"path": os.path.join(base_dir, "genbank/train"), "type": "DNA", "fraction": 0.8 if args.train_type == "mixed" else 0.5, "offset": 0},
                                    {"path": os.path.join(base_dir, "uniref100/val"), "type": "peptide", "fraction": 0.2 if args.train_type == "mixed" else 0.5, "offset": 0}],
                                    ctx_len=args.ctx_len, tokenizers=tokenizers)
    else:
    ...
    if logging and rank == 0:
        ...
        if needs_init:
            wandb.run.name = f"N{args.n_layer} {args.train_type}"
